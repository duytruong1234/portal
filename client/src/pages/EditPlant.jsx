...
const EditPlant = () => {
      ...
      <div className="min-h-screen bg-gray-50 py-8">
        <div className="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="bg-red-50 border border-red-200 rounded-lg p-8 text-center">
            ...
            <p className="text-red-700 mb-4">Chỉ admin mới có thể chỉnh sửa thực vật</p>
            <button
  ...
  const fetchPlant = async () => {
    try {
      ...
      if (response.data.success) {
        ...
        showError('Không tìm thấy thực vật')
        navigate('/plants')
      }
    } catch (err) {
      console.error('Error fetching plant:', err)
      showError(err.response?.data?.message || 'Lỗi khi tải thông tin thực vật')
      navigate('/plants')
    } finally {
      setLoading(false)
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setSaving(true)
      const response = await plantsApi.update(id, formData)
      
      if (response.data.success) {
        showSuccess('Cập nhật thực vật thành công!')
        navigate(`/plant/${id}`)
      }
    } catch (err) {
      console.error('Error updating plant:', err)
      showError(err.response?.data?.message || 'Lỗi khi cập nhật thực vật')
    } finally {
      setSaving(false)
    }
  }
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-8">
          ...
          >
            ...
            Quay lại chi tiết thực vật
          </button>
          
          <div className="flex items-center">
            <FaEdit className="text-3xl text-primary-600 mr-3" />
            <div>
              <h1 className="text-3xl md:text-4xl font-bold text-gray-900">
                Chỉnh sửa thực vật
              </h1>
              <p className="text-gray-600 mt-1">
                Cập nhật thông tin chi tiết về thực vật
              </p>
            </div>
          </div>
        </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="flex justify-end space-x-4 pt-6 border-t">
              ...
              >
                ...
                {saving ? 'Đang lưu...' : 'Cập nhật thực vật'}
              </button>
            </div>
          </form>
        </div>
